...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    <table class="shop_table cart cart-table wishlist_table responsive-table" cellspacing="0">
        ...
        <tr>
            ...
            <th class="product-name"><span class="nobr"><?php _e( 'Product Name', 'chow' ) ?></span></th>
            <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?><th class="product-price"><span class="nobr"><?php _e( 'Unit Price', 'chow' ) ?></span></th><?php endif ?>
            <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th><span class="nobr"><?php _e( 'Stock Status', 'chow' ) ?></span></th><?php endif ?>
        ...
        if( count( $wishlist ) > 0 ) :
            foreach( $wishlist as $values ) :
                ...
                if( $product_obj !== false && $product_obj->exists() ) : ?>
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        ...
                        <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?>
                            <td class="product-price">
                                ...
                                if( $product_obj->price != '0' ) {
                                    ...
                                    echo apply_filters( 'yith_free_text', __( 'Free!', 'chow' ) );
                                }
                                ?>
                            </td>
                        <?php endif ?>
                        <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?>
                            <td class="product-stock-status">
                                ...
                                if( $stock_status == 'out-of-stock' ) {
                                    $stock_status = "Out";
                                    echo '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'chow' ) . '</span>';
                                } else {
                                    $stock_status = "In";
                                    echo '<span class="wishlist-in-stock">' . __( 'In Stock', 'chow' ) . '</span>';
                                }
                                ?>
                            </td>
                        <?php endif ?>
                        ...
                        <?php endif ?>
                            <td class="product-remove"><a href="javascript:void(0)" onclick="<?php echo $remove_wishlist ?>" class="remove cart-remove" title="<?php _e( 'Remove this product', 'chow' ) ?>"></a></td>
                    </tr>
                <?php
                endif;
            endforeach;
            ...
            <tr>
                <td colspan="6" class="wishlist-empty"><?php _e( 'No products were added to the wishlist', 'chow' ) ?></td>
            </tr>
        <?php
        endif;
