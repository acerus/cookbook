...
$post_types      = array( 'attachment', 'wpcf7_contact_form', 'post', 'page','recipe' );
...
//			print_r($meta); exit;
		...
		if ( $post_type == 'attachment' ) {
			...
			if ( is_file( $file ) ) {
				...
				if ( is_dir( get_home_path() . '/wp-content/uploads/chow/images/stock/' ) ) {
					copy( $file, get_home_path() . '/wp-content/uploads/chow/images/stock/' . basename( $file ) );
				}
			}
...
if ( is_dir( get_home_path() . '/wp-content/uploads/chow/' ) ) {
	echo "it exists";
	file_put_contents( get_home_path() . '/wp-content/uploads/chow/default.json' , json_encode( $default_content ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/chow/widget_positions.json' , json_encode( $widget_positions ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/chow/widget_options.json' , json_encode( $widget_options ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/chow/menu.json' , json_encode( $menu_ids ) );
	file_put_contents( get_home_path() . '/wp-content/uploads/chow/options.json' , json_encode( $my_options ) );
}
