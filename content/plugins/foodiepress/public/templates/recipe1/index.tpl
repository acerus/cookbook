<div class="foodiepress-wrapper {$style} print-only ">
    <section class="foodiepress"  itemscope itemtype="http://schema.org/Recipe">
        ...
        <p class="author-data">{$lang.by}
            <a class="recipe-author" href="{$authorurl}" itemprop="author" itemscope itemtype="http://schema.org/Person"><span itemprop="name"> {$author} </span></a>
        ...
        <ul class="ingredients">
            {foreach $ingredients item}
                ...
                {else}
                    <li><span><a  target="_blank" href="{$item.url}" itemprop="ingredients">{$item.name}</a> {if $item.note} - {$item.note} {/if}</span></li>
                {/if}
            {/foreach}
        </ul>
